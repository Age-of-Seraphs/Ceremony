...
namespace CakeBuild
{
    ...
    {
        public const string ProjectName = "Ceremony";
    ...
    public sealed class PackageTask : FrostingTask<BuildContext>
    {
        public override void Run(BuildContext context)
        {
            ...
            // ---- Install to Vintage Story Mods folder (unpacked) ----
            var appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
            var vsModsDir = Path.Combine(appData, "VintagestoryData", "Mods");
            var installDir = Path.Combine(vsModsDir, context.Name);

            context.EnsureDirectoryExists(vsModsDir);

            // Delete old install to avoid stale files
            if (context.DirectoryExists(installDir))
            {
                context.DeleteDirectory(installDir, new DeleteDirectorySettings
                {
                    Recursive = true,
                    Force = true
                });
            ...
            // Copy freshly packaged folder into Mods/<modid>
            context.CopyDirectory($"../Releases/{context.Name}", installDir);
        }
    }
